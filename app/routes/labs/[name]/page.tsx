...
const LabsAssignmentPage = () => {
  ...
  useEffect(() => {
    getAssignmentByName("labs", name).then((val) => { setDetails(val); return val}).then((details) => markdownToHtml(details?.content || "").then((data) => setContent(data)));
  ...
  return (
    <PageContainer>
      ...
      <Container className="flex-col">
        <div className="flex-row flex py-2"><BackButton /><p className="text-3xl font-bold">Lab: {toTitleCase(name)}</p></div>
...
export default LabsAssignmentPage;
