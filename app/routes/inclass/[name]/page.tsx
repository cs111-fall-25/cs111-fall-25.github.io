...
const InclassAssignmentPage = () => {
  ...
  useEffect(() => {
    getAssignmentByName("inclass", name).then((val) => { setDetails(val); return val}).then((details) => markdownToHtml(details?.content || "").then((data) => setContent(data)));
  ...
  return (
    <PageContainer>
      ...
      <Container className="flex-col">
        <div className="flex-row flex py-2"><BackButton /><p className="text-3xl font-bold">In-class Assignment: {toTitleCase(name)}</p></div>
...
export default InclassAssignmentPage;
